export default {
  nav: {
    hub: 'Hub',
    profile: 'Profile',
    signOut: 'Sign Out',
    signIn: 'Sign In',
    signUp: 'Sign Up',
  },
  hub: {
    badge: '◆ Unofficial NosTale Companion ◆',
    heroSub: 'The social hub for Arcadia adventurers.\nProfile, raids, market and more — all in one place.',
    myProfile: 'My Profile',
    startAdventure: 'Start Adventure',
    signIn: 'Sign In',
    players: 'Players',
    guilds: 'Guilds',
    raidsDone: 'Raids Completed',
    features: 'Features',
    comingSoon: 'Soon',
    news: 'News',
    topPlayers: 'Top Players',
    heroLevel: 'Hero',
    lv: 'Lv.',
    newsLoading:    'Loading news…',
    newsEmpty:      'No news available.',
    featureProfile: { label: 'Profile',  desc: 'Your character, your stats, your equipment' },
    featureRaids:   { label: 'Raids',    desc: 'Plan and join raids with your guild' },
    featureMarket:  { label: 'Market',   desc: 'Buy and sell equipment between players' },
    featureGuild:   { label: 'Guild',    desc: 'Manage your guild and members' },
    featureRanking: { label: 'Ranking',  desc: 'Discover the top players on the server' },
    featureEvents:  { label: 'Events',   desc: 'Follow ongoing and upcoming events' },
  },
  auth: {
    signIn: 'Sign In',
    signUp: 'Sign Up',
    subtitleLogin:    'Find your adventurer in Arcadia.',
    subtitleRegister: 'Join the NosTale community.',
    adventurerName:   'Adventurer Name',
    email:            'Email Address',
    password:         'Password',
    confirmPassword:  'Confirm Password',
    minPassword:      'Min. 8 characters',
    btnSignIn:        'Sign In',
    btnCreate:        'Create Account',
    loading:          'Loading…',
    forgotPassword:   'Forgot password?',
    resetLink:        'Reset',
    noAccount:        "Don't have an account?",
    alreadyAccount:   'Already have an account?',
    success:          'Account created! Check your email to confirm your registration.',
    errRequired:      'Please fill in all required fields.',
    errUsernameReq:   'Adventurer name is required.',
    errUsernameLen:   'Name must be at least 3 characters.',
    errPasswordLen:   'Password must be at least 8 characters.',
    errPasswordMatch: 'Passwords do not match.',
    errCredentials:   'Invalid email or password.',
    errGeneric:       'An error occurred.',
  },
  profile: {
    myCharacters:   'My Characters',
    addCharacter:   'Add a character',
    lv:             'Lv.',
    heroShort:      'H.',
    heroLevel:      'Hero',
    about:          'About',
    statistics:     'Statistics',
    achievements:   'Achievements',
    equipment:      'Equipment',
    recentActivity: 'Recent Activity',
    since:          'Since',
    signIn:         'Sign In',
    edit:           '✏️ Edit',
    addFriend:        '➕ Friend',
    loginRequired:    'Sign in to access your profile',
    loginRequiredSub: 'Create an account and start building your adventurer.',
    noCharacter:      'Select a slot above to create your first character.',
  },
  notFound: {
    title:     'Page Not Found',
    sub:       "This area of Arcadia doesn't exist yet… or has been swallowed by darkness.",
    backToHub: 'Back to Hub',
  },
  stats: {
    speed: '💨 Speed',
    crit:  '🎯 Critical',
  },
  classes: {
    Archer:    'Archer',
    Swordsman: 'Swordsman',
    Mage:      'Mage',
    Martial:   'Martial Artist',
  },
  create: {
    title:         'Create a Character',
    nameLabel:     'Name',
    namePlaceholder: 'Adventurer name',
    classLabel:    'Class',
    levelLabel:    'Level',
    heroLevelLabel:'Hero Level',
    submit:        'Create',
    cancel:        'Cancel',
    errName:       'Name is required.',
    errNameLen:    'Name must be at least 3 characters.',
    errLevel:      'Level must be between 1 and 99.',
  },
  tabs: {
    equipment:   'Equipment',
    specialists: 'Specialists',
    fairies:     'Fairies',
    books:       'Books',
    booksSoon:   'Books coming soon.',
  },
  statKeys: {
    atk:      '⚔️ ATK',
    def:      '🛡️ DEF',
    matk:     '🔮 MATK',
    mdef:     '🧿 MDEF',
    hp:       '❤️ HP',
    mp:       '💧 MP',
    speed:    '💨 Speed',
    critRate: '🎯 Crit Rate',
    critDmg:  '💥 Crit Dmg',
    hit:      '🎪 Hit',
    avoid:    '🌀 Avoid',
  },
  equipKeys: {
    weapon:   '🗡️ Weapon',
    offhand:  '🛡️ Off-hand',
    armor:    '🧥 Armor',
    hat:      '🎩 Hat',
    gloves:   '🧤 Gloves',
    shoes:    '👟 Shoes',
    necklace: '📿 Necklace',
    ring:     '💍 Ring',
    bracelet: '⌚ Bracelet',
    sp:       '🏹 SP Card',
    fairy:    '🦋 Fairy',
    empty:    'Empty',
  },
  elements: {
    mainElement:  'Main Element',
    resistances:  'Resistances',
    neutral:  'Neutral',
    fire:     'Fire',
    water:    'Water',
    light:    'Light',
    shadow:   'Shadow',
  },
  players: {
    title:          'Player Search',
    sub:            'Find adventurers from Arcadia and explore their profiles.',
    myProfile:      'My Profile',
    searchPlaceholder: 'Search a player…',
    noResults:      'No players found.',
    lv:             'Lv.',
    hero:           'Hero',
    back:           '← Back to search',
    notFound:       'Player not found',
    notFoundSub:    'This adventurer does not seem to exist in Arcadia.',
    chars:          'characters',
  },
}
